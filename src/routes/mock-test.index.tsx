...
import { fetchSubjects, fetchSubjectQuestionCount, type DbSubject } from "@/lib/pyq/db";
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const pairs = await Promise.all(
        subs.map(async (s) => [s.id, await fetchSubjectQuestionCount(s.id)] as const),
      );
      if (cancelled) return;
          ...
          <div className="bg-hero relative overflow-hidden rounded-3xl border border-white/5 p-5 shadow-card-premium">
            ...
            <div className="relative">
              ...
              <div className="mt-3 inline-flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <Timer className="h-3 w-3" /> Suggested time: 120 minutes
              </div>
            </div>
          </div>
