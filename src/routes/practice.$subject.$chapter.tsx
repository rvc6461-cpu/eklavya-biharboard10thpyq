...
export const Route = createFileRoute("/practice/$subject/$chapter")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.chapter.name ?? "Practice"} · Eklavya` },
      { name: "description", content: `Practice PYQs from ${loaderData?.chapter.name ?? ""}. Offline & chapter-wise.` },
    ],
  }),
    ...
    <div className="min-h-screen bg-background pb-28 text-foreground">
      <div className="mx-auto max-w-md">
        <header className="flex items-center justify-between gap-3 px-5 pt-6 pb-4">
          <button
            onClick={() => navigate({ to: "/practice/$subject", params: { subject: subject.id } })}
      ...
      <div className="mx-auto max-w-md space-y-5">
        <div className="bg-hero relative overflow-hidden rounded-3xl border border-white/5 p-6 text-center shadow-card-premium">
          ...
          <div className="relative">
            ...
            <h2 className="font-display mt-4 text-2xl font-bold">Session complete</h2>
        ...
        <button
          ...
          className="bg-gradient-primary shadow-glow flex w-full items-center justify-center gap-2 rounded-2xl py-4 font-display font-bold text-primary-foreground"
        >
          <RotateCcw className="h-5 w-5" /> Retry chapter
        </button>
